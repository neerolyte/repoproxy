...
function ProxyServer(options, requestListener) {
...
util.inherits(ReverseProxyServer, ProxyServer);
exports.ReverseProxyServer = ReverseProxyServer;

function YumProxyServer(options, requestListener) {
	if (!(this instanceof YumProxyServer)) return new YumProxyServer(requestListener);
	http.Server.call(this);
...
util.inherits(YumProxyServer, ProxyServer);
exports.YumProxyServer = YumProxyServer;

exports.createServer = function(options, requestListener) {
	var proxytype = ReverseProxyServer;
	if (options.proxy.type) {
		if (this[options.proxy.type]) {
			proxytype = this[options.proxy.type];
		} else {
			throw "No such proxy type: " + options.proxy.type;
		}
	}
