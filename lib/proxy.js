...
Proxy.prototype.application = function(request) {
	...
	if (this._active[request.url]) {
		cacheProm = this._active[request.url].cacheProm;
	} else {
		cacheProm = this._cacher.getCacheFile(request.url);
		this._active[request.url] = { 
			cacheProm: cacheProm,
			request: request,
		};
	}

	return cacheProm.then(function(cacheFile) {
		if (cacheFile) {
 ...
 */
Proxy.prototype._appCacheable = function(request, cacheFile, first) {
	var self = this;
