...
Proxy.prototype._appCacheable = function(request, cacheFile) {
	...
	if (!this._collapsible[request.url]) {
		this._collapsible[request.url] = cacheFile;
	} else {
		// or we should collapse this request
		return this._appCollapse(request, this._collapsible[request.url]);
	}
...
Proxy.prototype._appCacheFromUpstream = function(request, cacheFile) {
	...
	return Q.all([
		...
		var req = HTTP.request(request.url)
...
Proxy.prototype._removeCompletedRequests = function() {
	this._active = _.reject(this._active, function(req) {
