...
Server.prototype.tryCache = function(req, res) {
	...
	fs.open(req.url, 'r', 0666, function(err, fd) {
		self.triedCache(req, res, err, fd);
	});
};

Server.prototype.triedCache = function(req, res, err, fd) {
...
Server.prototype.update = function(req, res) {
	console.log("now updating " + req.url)
	throw new Error('I really should write some code here.');
};

Server.prototype.getStorePath = function(data) {
	return this.options.cacheDir + data.path;
};
