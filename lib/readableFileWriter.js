...
ReadableFileWriter.prototype.move = function(dest) {
	oldPath = this._file;
	this._file = dest;
	return FS.move(oldPath, dest);
}
...
WriterReader.prototype.forEach = function(cb) {
	if (this._writer._offset == this._offset) {
		if (this._writer.closed) return Q();
...
WriterReader.prototype._playCatchUp = function(cb) {
	var file;
	var self = this;
	return FS.open(this._writer._file, { 
		flags: "rb",
		begin: this._offset,
		end: this._writer._offset,
	})
	.then(function(f) {
		file = f;
