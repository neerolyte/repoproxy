...
/**
 * Index the contents of an Apt Packages file
 *
 * @param str contents of Release file
 */
RepoApt.prototype.indexPackages = function(str) {
	// currently just look for 'Filename: *' lines and store those
	// TODO: convert from ham-fisted string manipulation, to stream parsing
	var lines = str.split('\n');
	...
	for (var i in lines) {
		...
		if (line.indexOf('Filename: ') == 0) {
			this.addFile(line.substring('Filename: '.length));
		}
	}
};